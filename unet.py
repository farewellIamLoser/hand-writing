...
class DecoderBlock(nn.Module):
    ...
    def __init__(self,in_channels,out_channels,time_embedding_dim):
        ...
        # 1. 使用上采样将特征图的大小增大
        # self.upsample = nn.Upsample(scale_factor=2, mode='nearest')  # 或者使用 ConvTranspose2d
        self.upsample = nn.ConvTranspose2d(
            in_channels//2, in_channels//2, kernel_size=4, stride=2, padding=1
        )
        # 2. 跳跃连接合并后的卷积层
        self.conv = nn.Sequential(
            ConvBnSiLu(in_channels, out_channels // 2, 3, 1, 1),
        )
    ...
    def forward(self,x,x_shortcut,t=None):
        ...
        # 2. 跳跃连接合并
        x = torch.cat([x, x_shortcut], dim=1)  # Concatenate along channels
