...
class Diffuser(nn.Module):
    def __init__(self,image_size,in_channels,time_embedding_dim=100,timesteps=1000,base_dim=32,dim_mults= [1, 2, 4, 8]):
        super().__init__()
    ...
    def forward(self, x, noise):
        ...
        pred_noise = self.model(x_t, t.long())
    ...
    def _linear_variance_schedule(self, timesteps: int):
        ...
        beta_end = scale * 0.02
        betas = torch.linspace(beta_start,beta_end,timesteps).astype(torch.float())
        return betas
        # ---------- **** ---------- #
